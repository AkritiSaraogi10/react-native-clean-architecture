...
class PostService {
  ...
  private constructor() {
    ...
    this.realmService = RealmService.getInstance<PostSchema>([PostSchema.schema]);
  }
  ...
  async getPosts(listener: CollectionChangeCallback<PostSchema, [number, PostSchema]>): Promise<IPost[]> {
    // Fetching all posts from Realm database
    const data = this.realmService.fetchAllObjectsFromRealm(PostSchema.schema.name);
